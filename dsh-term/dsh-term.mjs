...
async function taskFromFigma(opts, parts, promptTurnFn, state) {
  ...
  const prompt = [
    ...
    `3) sltracker: create_task(queue="${queue}", title=имя кадра, description=факты дизайна числами)`,
    '4) sltracker: add_comment(key из ответа create_task, ссылка на кадр + ключевые числа)',
    '5) sltracker: get_task(key из create_task) — проверить, что описание и комментарий на месте',
    '',
