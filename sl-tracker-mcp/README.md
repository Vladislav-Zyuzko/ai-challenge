...
| `list_issues` | задачи очереди с фильтром по статусам (ключи или имена) | `GET /api/queues/{key}/issues` |
| `list_comments` | комментарии задачи: автор, дата, текст, `total` | `GET /api/issues/{key}/comments` |
| `list_queues` | справочник: проекты → очереди → статусы (`ключ=«имя»`), можно по одному проекту | `GET /api/projects…`, `/api/queues/{key}/statuses` |

Пять операций — из задания. Остальные три закрывают то, без чего агент угадывал бы данные:
`list_queues` — ключи очередей и имена статусов, `list_issues` — выборка по статусам (списком,
а не по одному ключу), `list_comments` — признак живого обсуждения в задаче. Последние два
появились под сервис дайджеста из `sl-claude-box`: он собирает активные задачи по расписанию
и именно так узнаёт, что происходит в трекере.
